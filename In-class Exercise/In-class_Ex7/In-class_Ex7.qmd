---
title: "7: Calibrating Hedonic Pricing Model for Private Highrise Property with GWR Method - Using AIC method"
...
The result from AIC shows that the recommended bandwidth is 1391.309 metres.
...
The report shows that the AICc of the gwr is 42254.7 which is smaller than the AICc of the multiple linear regression model derived from CV method (42263.61).

Additionally, adjusted R-square value from AIC method is 0.818, which is smaller than the adjusted R-square value from the CV method (0.843).
...
The result shows that 27 is the recommended data points to be used.
...
The report shows that the AICc the adaptive distance gwr is 41978.63 which is even smaller than the AICc of the fixed distance gwr (42254.7), and the AICc of the adaptive distance gwr using the CV method (41982.22).

The adjusted R-square value for adaptive distance gwr is 0.863, higher than the adjusted R-square value for fixed distance gwr (0.818) and even higher than the adjusted R-square value for adaptive distance gwr using CV method (0.856).
...
We create a static point symbol map.
...
Adaptive GWR models produced nearly identical spatial patterns of local R², regardless of whether AIC or CV was used for bandwidth selection.

The similarity occurs because AIC and CV, while different in computation, share the same fundamental goal of optimising the trade-off between model fit and complexity to avoid overfitting.
