---
title: "2b: 2nd Order Spatial Point Patterns Analysis Methods"
...
The $G_{\text{bord}}(r)$ line rises earlier than the $G_{\text{pois}}(r)$ (CSR) line, indicating that many points have very close neighbours and that clustering exists.
...
As $G_{\text{obs}}(r)$ lies above the grey envelope for shorter distances, it indicates that the clustering effect is mainly local.
...
The gap between $G_{\text{bord}}(r)$ and $G_{\text{pois}}(r)$ (CRS) lines are consistently wide, showing a regular pattern. There is a steep increase at shorter distances, hence local clustering may be present.
...
As $G_{\text{obs}}(r)$ goes above the grey envelope in short distances, local clustering is observed.
...
`Fest()` : Computes F-function estimation `envelope()` : Performs monte carlo simulation test
...
$F_{km}(r)$ aligned with $F_{pois}(r)$, indicating that it is close to CSR.
...
The $F_{\text{obs}}(r)$ line goes below the envelope for large distances, indicating that the points are clustered.

## 5 Analysing Spatial Point Process Using K-Function

**K function**: Measures the number of events found up to a given distance of any particular event.

`Kest()` : Computes K-function estimation `envelope()` : Performs monte carlo simulation test
...
`Lest()` : Computes L-function estimation `envelope()` : Performs monte carlo simulation test
